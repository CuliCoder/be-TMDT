...
export const registerUser = async (fullname, email, phonenumber, password, confirmPassword) => {
  ...
  const [existingUsers] = await database.execute(checkQuery, [email, phonenumber]);
  ...
  await database.execute(query, [fullname, email, phonenumber, hashedPassword, 'customer', 1]);
...
export const loginUser = async (identifier, password) => {
  const query = `SELECT * FROM users WHERE (Email = ? OR PhoneNumber = ?) AND Status = 1`;
  ...
  if (!bcrypt.compareSync(password, user.PasswordHash)) {
      throw { error: 1, message: "Mật khẩu không chính xác" };
  }
