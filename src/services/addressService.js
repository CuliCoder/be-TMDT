...
export const addAddress = async (
  ...
  try {
    const query =
      "INSERT INTO user_address (iduser, phonenumber, name, address, setDefault) VALUES (?, ?, ?, ?, ?)";
...
export const updateAddress = async (id, name, phonenumber, address) => {
  try {
    const query =
      "UPDATE user_address SET name = ?, phonenumber = ?, address = ? WHERE iduser_address = ?";
    const [result] = await connection.execute(query, [
      name,
      phonenumber,
      address,
...
export const setDefaultAddress = async (iduser_address, iduser) => {
  try {
    ...
    await connection.execute(
      "UPDATE user_address SET setDefault = 0 WHERE iduser = ?",
      [iduser]
    );
    // Đặt địa chỉ được chọn làm mặc định
    await connection.execute(
      "UPDATE user_address SET setDefault = 1 WHERE iduser_address = ?",
      [iduser_address]
    );
  } catch (error) {
    console.error(error);
  ...
  new Promise(async (resolve, reject) => {
    try {
      const [address] = await connection.execute(
        "SELECT * FROM user_address WHERE iduser = ? AND setDefault = 1",
        [iduser]
      );
      if (address.length == 0) {
        resolve(null);
